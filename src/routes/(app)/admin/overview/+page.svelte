...
<div class="space-y-6">
	...
	{:else}
		...
		{#if data.admin.exchangeRates?.length}
			<Card.Root>
				<Card.Header>
					<Card.Title class="text-base">Exchange Rates</Card.Title>
				</Card.Header>
				<Card.Content>
					<div class="border-border overflow-x-auto rounded-md border">
						<table class="w-full text-sm">
							<thead>
								<tr class="border-border bg-muted/50 border-b">
									<th class="px-4 py-2 text-left font-medium">Pair</th>
							...
							<tbody>
								{#each data.admin.exchangeRates as rate (rate.label)}
									<tr class="border-border border-b last:border-0">
										<td class="px-4 py-2 font-mono text-xs">{rate.label}</td>
		...
		<Card.Root>
			<Card.Header>
				<Card.Title class="text-base">Housekeeping</Card.Title>
			</Card.Header>
			<Card.Content>
